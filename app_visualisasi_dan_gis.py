...
if df.empty:
    st.error("⚠️ File 'curah_hujan_bandung.xlsx' tidak ditemukan.")
    st.stop()
...
with st.sidebar:
    ...
    with st.container(border=True):
        ...
        if mode_filter == "Per-Tahun":
            ...
            start_date = pd.Timestamp(f"{pilihan_tahun}-01-01")
            end_date = pd.Timestamp(f"{pilihan_tahun}-12-31")
            ...
            if input_start > input_end:
                st.error("Error: Tanggal Mulai > Selesai")
                st.stop()
...
metrics_config = [
    ("Rata-rata Hujan", filtered_df['Curah Hujan (mm)'].mean(), "mm", COLORS['hujan'], m1),
    ("Hujan Tertinggi", filtered_df['Curah Hujan (mm)'].max(), "mm", COLORS['hujan'], m2),
    ("Rata-rata Angin", filtered_df['Kecepatan Angin (km/h)'].mean(), "km/h", COLORS['angin'], m3),
...
def make_plotly_chart(data, y_col, color_hex, chart_type='bar'):
    if data.empty: return px.bar(title="Data Kosong")
    if chart_type == 'bar':
        fig = px.bar(data, x='Tanggal', y=y_col, color_discrete_sequence=[color_hex], template='plotly_dark')
    else:
        fig = px.line(data, x='Tanggal', y=y_col, color_discrete_sequence=[color_hex], template='plotly_dark')
        r, g, b = tuple(int(color_hex.lstrip('#')[i:i+2], 16) for i in (0, 2, 4))
        fig.update_traces(fill='tozeroy', fillcolor=f"rgba({r},{g},{b},0.2)")
    fig.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(t=20, l=0, r=0, b=0), hovermode="x unified")
    return fig

tab1, tab2, tab3, tab4, tab5 = st.tabs(["🌧️ Tren Hujan", "💨 Tren Angin", "🌡️ Tren Suhu", "📊 Distribusi", "📂 Data Mentah"])

with tab1: st.plotly_chart(make_plotly_chart(filtered_df, 'Curah Hujan (mm)', COLORS['hujan'], 'bar'), use_container_width=True)
with tab2: st.plotly_chart(make_plotly_chart(filtered_df, 'Kecepatan Angin (km/h)', COLORS['angin'], 'line'), use_container_width=True)
with tab3: st.plotly_chart(make_plotly_chart(filtered_df, 'Suhu rata-rata (°C)', COLORS['suhu'], 'line'), use_container_width=True)
with tab4:
    if not filtered_df.empty:
        cnt = filtered_df['Intensitas'].value_counts().reset_index()
        cnt.columns = ['Intensitas', 'Jumlah Hari']
        fig_dist = px.bar(cnt, x='Jumlah Hari', y='Intensitas', orientation='h', color='Intensitas', color_discrete_map=COLORS['intensitas'], template='plotly_dark')
        fig_dist.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', showlegend=False)
...
if show_map:
    ...
    angin_min = filtered_df['Kecepatan Angin (km/h)'].min()
    angin_max = filtered_df['Kecepatan Angin (km/h)'].max()
    rata_suhu_display = f"{filtered_df['Suhu rata-rata (°C)'].mean():.1f}" if not filtered_df.empty else "0.0"
            ...
            <div style="padding: 15px; background-color: white; border-radius: 0 0 8px 8px;">
                <table style="width: 100%; font-size: 12px; border-collapse: collapse;">
                    <tr style="border-bottom: 1px solid #f2f2f2;">
                        <td style="padding: 6px 0; width: 140px;"><b>Periode</b></td>
                        <td style="width: 10px;">:</td>
                        <td style="text-align: right;">{note_text}</td>
                    </tr>
                    <tr style="border-bottom: 1px solid #f2f2f2;">
                        <td style="padding: 6px 0;"><b>Total Curah Hujan</b></td>
                        <td>:</td>
                        <td style="text-align: right;">{total_hujan:.1f} mm</td>
                    </tr>
                    <tr style="border-bottom: 1px solid #f2f2f2;">
                        <td style="padding: 6px 0;"><b>Angin Min/Max</b></td>
                        <td>:</td>
                        <td style="text-align: right;">{angin_min:.1f} - {angin_max:.1f} km/h</td>
                    </tr>
                    <tr style="border-bottom: 1px solid #f2f2f2;">
                        <td style="padding: 6px 0;"><b>Suhu Rata-rata</b></td>
                        <td>:</td>
                        <td style="text-align: right;">{rata_suhu_display} °C</td>
                    </tr>
                    <tr>
                        <td style="padding: 6px 0;"><b>Total Hari</b></td>
                        <td>:</td>
                        <td style="text-align: right;">{total_hari} Hari</td>
                    </tr>
                </table>
                <div style="margin-top: 10px; padding: 6px; background: {warna_marker}; color: white; 
